...
class MigrateDependenciesTest extends MigrateDrupal6TestBase {
  ...
  public function testMigrateDependenciesOrder() {
    ...
    // migrate dependencies for comment include dependencies for node
    // migration as well.
  ...
  public function testAggregatorMigrateDependencies() {
    ...
    $executable->import();
    $this->assertEqual($this->migrateMessages['error'], [SafeMarkup::format('migrate @id did not meet the requirements. Missing migrations d6_aggregator_feed. requirements: d6_aggregator_feed.', ['@id' => $migration->id()])]);
