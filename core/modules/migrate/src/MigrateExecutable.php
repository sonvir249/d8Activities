...
class MigrateExecutable implements MigrateExecutableInterface {
  ...
  public function import() {
    ...
    if ($this->migration->getStatus() !== MigrationInterface::STATUS_IDLE) {
      $this->message->display($this->t('migrate @id is busy with another operation: @status',
        [
    ...
    try {
      ...
      $this->message->display(
        $this->t(
          'migrate @id did not meet the requirements. @message @requirements',
          [
            '@id' => $this->migration->id(),
    ...
    try {
      ...
      $this->message->display(
        $this->t('migrate failed with source plugin exception: @e', ['@e' => $e->getMessage()]), 'error');
    ...
    while ($source->valid()) {
      ...
      try {
        ...
        $this->message->display(
          $this->t('migrate failed with source plugin exception: @e',
  ...
  public function rollback() {
    ...
    if ($this->migration->getStatus() !== MigrationInterface::STATUS_IDLE) {
      $this->message->display($this->t('migrate @id is busy with another operation: @status', ['@id' => $this->migration->id(), '@status' => $this->t($this->migration->getStatusLabel())]), 'error');
      return MigrationInterface::RESULT_FAILED;
    }
