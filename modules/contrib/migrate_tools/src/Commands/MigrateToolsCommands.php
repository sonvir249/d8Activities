...
class MigrateToolsCommands extends DrushCommands {
  ...
  /**
   * migrate plugin manager service.
   *
    ...
    if ($migration) {
      ...
      switch ($status) {
        case MigrationInterface::STATUS_IDLE:
          $this->logger()->warning(
            dt('migrate @id is idle', ['@id' => $migration_id])
          );
          break;

        case MigrationInterface::STATUS_DISABLED:
          $this->logger()->warning(
            dt('migrate @id is disabled', ['@id' => $migration_id])
          );
          break;

        case MigrationInterface::STATUS_STOPPING:
          $this->logger()->warning(
            dt('migrate @id is already stopping', ['@id' => $migration_id])
          );
          break;
        ...
        default:
          ...
          $this->logger()->notice(
            dt('migrate @id requested to stop', ['@id' => $migration_id])
          );
          break;
      }
    }
      ...
      $this->logger()->error(
        dt('migrate @id does not exist', ['@id' => $migration_id])
      );
    }
  }
  ...
  public function resetStatus($migration_id = '') {
    ...
    if ($migration) {
      ...
      if ($status == MigrationInterface::STATUS_IDLE) {
        $this->logger()->warning(
          dt('migrate @id is already Idle', ['@id' => $migration_id])
        );
      }
      else {
        $migration->setStatus(MigrationInterface::STATUS_IDLE);
        $this->logger()->notice(
          dt('migrate @id reset to Idle', ['@id' => $migration_id])
        );
      }
    }
    else {
